...
def clean_and_extract_answers(text):
...
def process_pdfs(answer_key_path, student_answers_path):
    ...
    correct_answers = clean_and_extract_answers(correct_answers_text)
    student_answers = clean_and_extract_answers(student_answers_text)
    ...
    for i, (correct_answer, student_answer) in enumerate(zip(correct_answers, student_answers), start=1):
        ...
        results[str(i)] = (student_answer, correct_answer, round(adjusted_similarity * 100, 2), grade)

    average_similarity = total_similarity / len(correct_answers)
...
def upload_student():
    ...
    results, final_grade = process_pdfs(answer_key_path, student_answers_path)
